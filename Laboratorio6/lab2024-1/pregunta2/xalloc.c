	...
	*/
	...
	Header *bestFit = NULL;
 	for (p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
        if (p->s.size >= nunits){ 
            if (p->s.size == nunits){
                prevp->s.ptr = p->s.ptr; //saltamos ya que cubre todo
                return (void *)(p + 1);
            }
            else{ 
                if (bestFit == NULL || bestFit->s.size > p->s.size)
					bestFit = p;
            }
        }
		// Dio una vuelta
        if (p == freep){
            if(bestFit!=NULL){
				printf("El campo best fit es %p con size=%zu units\n",bestFit,bestFit->s.size);
                bestFit->s.size -= nunits;
                bestFit += bestFit->s.size;
                bestFit->s.size = nunits;
