...
void *xmalloc (size_t nbytes)
{
	...
	/*utilizamos el tamaño de asignación más pequeño Align*/
	nunits = (nbytes+sizeof(Align)-1)/sizeof(Align) + 1;
	...
	for (p= prevp->s.ptr;  ;prevp = p, p = p->s.ptr){
		if (p->s.size >= nunits) {  /* big enough */
			if (p->s.size == nunits)  /* exactly */
				...
				size_t condition = p->s.size - nunits;
				//only i
				if(condition >= 1){
				}
				p->s.size -= nunits;
...
void xfree(void *ap)
{
	...
	*/
	...
	/*Si los huecos están juntos se unen en uno solo*/
	if ((Align *)bp + bp->s.size == p->s.ptr) {
